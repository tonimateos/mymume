    ...
    const fetchPlaylist = async (inputUrl?: string) => {
        ...
        try {
            const res = await fetch("/api/playlist", {
                method: inputUrl ? "POST" : "GET",
                headers: { "Content-Type": "application/json" },
                body: inputUrl ? JSON.stringify({ url: inputUrl }) : undefined
            })

            if (!res.ok) {
                if (res.status === 404) {
                    // No playlist saved yet, just stay empty
                    setPlaylist(null)
                    return
                }
            ...
            const data = await res.json()
            // If GET returns { playlist: null }, handle it
            if (data.playlist === null) {
                setPlaylist(null)
            } else {
                setPlaylist(data)
                // If we just fetched successfully from an input URL, clear the input
                if (inputUrl) setUrl("")
            }
        } catch (err: any) {
            console.error(err)
            ...
            <main className="max-w-4xl mx-auto flex flex-col items-center gap-12">
                ...
                <div className="w-full max-w-xl text-center space-y-4">
                    ...
                    <p className="text-neutral-400">
                        {playlist ? "Replace it by entering a new URL below." : "Enter a Spotify playlist URL to get started."}
                    </p>
                    ...
                    <form onSubmit={handleSubmit} className="relative mt-6">
                        <input
                            ...
                            className="w-full px-6 py-4 bg-neutral-900 border border-neutral-800 rounded-full focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent text-lg placeholder-neutral-600 transition-all"
                        />
                        <button
                            type="submit"
